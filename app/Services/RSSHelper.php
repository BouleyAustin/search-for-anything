...
class RSSHelper
{
    ...
    {
        $rssFeed = FeedReader::read($rssUrl)->get_items();
        ...
        foreach($rssFeed as $item){
            ...
            if(!Content::where('podcast_link', $hostLink)->first()){
                ...
                $newContent->summary = strip_tags($description);
        ...
        $page->link_home = strlen($imageUrl) > 50 ? null : $imageUrl;
        $page->save();
    }

    public static function createEpisodesFromRussellBruson($rssUrl, $pageId)
    {
        $rssFeed = FeedReader::read($rssUrl)->get_items();
        ...
        foreach ($rssFeed as $item) {
            ...
            $position = strpos($description, '---Transcript---');
            $summary = strip_tags(substr($description, 0, $position));
            $transcript = substr($description, $position);

            $transcript = str_replace('<p>', '', $transcript);
            $transcript = str_replace('</p>', "\n", $transcript);
            $transcript = str_replace('---Transcript---', '', $transcript);

            $transcript = explode("\n", $transcript);

            $transcript = self::remove_emoji($transcript);
            $summary = self::remove_emoji($summary);
            ...
            if(!Content::where('podcast_link', $hostLink)->first()){
                ...
                $newContent->platform = 'podcast';
                $newContent->people = $author;
                $newContent->length = round($duration * 60, 2);
                $page->contents()->save($newContent);

                foreach($transcript as $section){
                    $newItem = new Transcription();
                    $newItem->sentence = $section;
                    $newItem->page_id = $newContent->page_id;
    ...
    public static function getLibsynLinks($pageId)
    {
        $contents = Page::where('id', $pageId)->first()->contents()->pluck('id')->toArray();

        foreach($contents as $content){

            $updateContent = Content::where('id', $content)->first();

            if($updateContent->tags == null){
                try{
                    $html = file_get_contents($updateContent->podcast_link);
                    $position = strpos($html, 'twitter:player');
                    $html = substr($html, $position);
                    $position = strpos($html, 'content=');
                    $html = substr($html, $position + 9);
                    $position = strpos($html, '/id');
                    $html = substr($html, $position + 4, 15);
                    $length = strpos($html, '/');
                    $code = substr($html, 0, $length);

                    $updateContent->tags = $code;
                    $updateContent->save();
    ...
    public static function getNewEpisodes()
    {
        $pageIds = Page::all()->pluck('id')->toArray();

        foreach($pageIds as $id){
            $page = Page::where('id', $id)->first();
            $rssUrl = $page->podcast_rss;
            $rssFeed = FeedReader::read($rssUrl)->get_items();

            $latest = $rssFeed[0];

            $title = $latest->get_title();
            $description = $latest->get_description();
            $description = self::remove_emoji($description);
            ...
            if(!Content::where('podcast_link', $hostLink)->first()) {
                ...
                $transcript = null;
                if(str_contains($description, '---Transcript---')){
                    $position = strpos($description, '---Transcript---');
                    $description = strip_tags(substr($description, 0, $position));
                    $transcript = substr($description, $position);
